...
def _build_system_prompt(tools_summary: str, skills_text: str) -> str:
    ...
    parts: list[str] = [
        ...
        "IRIS server tools accept optional ``target_host`` and ``target_port`` parameters.",
        "By default they connect to the configured IRIS server. When the user asks to",
        "run an operation on a **specific server** (e.g. “check the version on 10.0.0.5”,",
        "“query the production server at 192.168.1.100:52773”), pass the requested",
        "host and port explicitly:\n",
        "- ``target_host`` — hostname or IP (e.g. ``10.0.0.5``)",
        "- ``target_port`` — REST API port (e.g. ``52773``)",
        "- If the user mentions only the host, omit ``target_port`` (uses configured default).",
        "- If the user does not specify a server, **omit both** — never guess a target.\n",
        "## Shell commands\n",
