...
Or install from source (not yet published on PyPI):

```bash
git clone https://github.com/AdriaERNI/Prism.git
cd Prism
uv sync
# then run with: uv run prism --help
```
...
| `prism-X.Y.Z-py3-none-any.whl` | Python wheel (build via `uv build`; not on PyPI yet) |
